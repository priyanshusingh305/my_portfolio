		...
		<Card className="group hover:shadow-xl transition-all duration-300 border-0 shadow-md overflow-hidden">
			...
			<CardHeader className="pb-3">
				<CardTitle className="line-clamp-2 group-hover:text-blue-600 transition-colors text-xl leading-tight">
					<Link
						href={`/blog/category/${category?.slug || "uncategorized"}/tags/${
							tags.length > 0 ? tags.map((tag) => tag.slug).join(",") : "none"
						}/slug/${post.slug}`}
					>
						{post.title}
					</Link>
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
			{Array.from({ length: 6 }).map(() => {
				const uniqueKey = `loading-${Math.random().toString(36).substr(2, 9)}`;
				return (
			...
			<div className="container mx-auto px-4 py-8">
				...
				<div className="text-center mb-20">
					...
					<p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
						Welcome to my blog! Here you&apos;ll find articles on web development, design, and technology. Explore the
						latest posts, filter by categories or tags, and discover featured content.
					</p>
				</div>
						...
						<h2 className="text-3xl font-bold">
							{category && category !== "all"
								...
								: tag && tag !== "all"
									? `Posts tagged "${
											// biome-ignore lint/suspicious/noExplicitAny: <explanation>
											tags.find((t: any) => t.slug === tag)?.name || tag
										}"`
									: "Latest Posts"}
						</h2>
